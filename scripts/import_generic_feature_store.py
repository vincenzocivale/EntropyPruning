#!/usr/bin/env python
"""Import generic per-slide feature files into an EAF WSI feature store."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Import per-slide .pt/.npy/.npz WSI feature files into EAF."
    )
    ...
    parser.add_argument("--coords-key", type=str, default=None)
...
def _read_manifest(path: Path) -> tuple[GenericFeatureSlideRecord, ...]:
    base_dir = path.parent
    records: list[GenericFeatureSlideRecord] = []
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            records.append(
                GenericFeatureSlideRecord(
                    ...
                    metadata={
                        "generic_manifest": str(path),
                        "generic_manifest_row": row_number,
                    },
                )
            )
...
def main() -> int:
    ...
    for record in records:
        bag = load_generic_feature_slide_record(
            record,

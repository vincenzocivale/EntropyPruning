#!/usr/bin/env python
"""Train an ABMIL classifier from an HDF5 WSI feature store."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train an ABMIL classifier from an HDF5 WSI feature store."
    )
...
def main() -> None:
    ...
    best_path = args.output_dir / "best_abmil_classifier.pt"
    ...
    for epoch in range(1, args.epochs + 1):
        train_output = train_abmil_classification_epoch(
            model,
            train_loader,
            optimizer,
            device=device,
        ...
        val_output = evaluate_abmil_classification_epoch(
            model,
            val_loader,
            device=device,
        )

#!/usr/bin/env python
"""Build a manifest for generic per-slide WSI feature files."""
...
_ALLOWED_SUFFIXES = {".pt", ".pth", ".npy", ".npz"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a manifest from generic per-slide feature files."
    )
...
def _discover_files(directory: Path, glob_pattern: str) -> dict[str, Path]:
    ...
    if not records:
        raise ValueError(
            f"no supported feature files found in {directory} using glob {glob_pattern!r}; "
            f"supported suffixes are {sorted(_ALLOWED_SUFFIXES)}."
        )
...
def main() -> int:
    ...
    features = _discover_files(args.features_dir, args.feature_glob)

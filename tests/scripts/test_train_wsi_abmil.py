...
def test_train_wsi_abmil_cli_smoke(tmp_path) -> None:
    ...
    checkpoint_path = output_dir / "best_abmil_classifier.pt"
    summary_path = output_dir / "training_summary.json"

    assert checkpoint_path.exists()
    assert summary_path.exists()

    checkpoint = load_abmil_classifier_checkpoint(checkpoint_path)
    assert checkpoint.config.feature_dim == 8
    assert checkpoint.config.hidden_dim == 16
    assert checkpoint.config.n_classes == 2
    ...
    assert len(summary["split"]["train_slide_ids"]) == 6
    assert len(summary["split"]["val_slide_ids"]) == 2


def test_train_wsi_abmil_cli_supports_explicit_split_files(tmp_path) -> None:
    repo_root = Path(__file__).resolve().parents[2]
    feature_store_path = tmp_path / "features.h5"
    output_dir = tmp_path / "out"
...
def test_train_wsi_abmil_cli_supports_no_gated_flag(tmp_path) -> None:
    repo_root = Path(__file__).resolve().parents[2]
    feature_store_path = tmp_path / "features.h5"
    output_dir = tmp_path / "out"
    ...
    checkpoint = load_abmil_classifier_checkpoint(output_dir / "best_abmil_classifier.pt")
    assert checkpoint.config.gated is False


def test_train_wsi_abmil_cli_supports_split_dir(tmp_path) -> None:
    repo_root = Path(__file__).resolve().parents[2]
...
def test_train_wsi_abmil_cli_rejects_split_dir_with_explicit_split_files(tmp_path) -> None:
    repo_root = Path(__file__).resolve().parents[2]
